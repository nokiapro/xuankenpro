...
function renderDetail(item) {
            ...
            <div class="collapse-box comment-box" data-collapse="comments">
              ...
              <div class="collapse-body" hidden>
                <div class="comment-form">
                  <div class="cmt-reply-bar" id="cmtReplyBar">
                    <span id="cmtReplyLabel">Đang trả lời...</span>
                    <button type="button" id="cmtReplyCancel" title="Hủy trả lời">&times;</button>
                  </div>
                  <input type="hidden" id="cmtReplyToId" value="" />
                  <input type="hidden" id="cmtReplyToName" value="" />
                  ...
                  <textarea id="cmtText" placeholder="Viết bình luận... (dùng @tên để tag)" rows="3" maxlength="500"></textarea>
  ...
  const replyCancel = document.getElementById("cmtReplyCancel");
  if (replyCancel) {
...
async function getAdminIdentitySet() {
  const emails = new Set();
  const names = new Set();
  try {
    if (firebaseAuth && firebaseAuth.currentUser && !firebaseAuth.currentUser.isAnonymous && firebaseAuth.currentUser.email) {
      emails.add(String(firebaseAuth.currentUser.email).toLowerCase().trim());
      if (firebaseAuth.currentUser.displayName) {
        names.add(String(firebaseAuth.currentUser.displayName).toLowerCase().trim());
      }
    }
  } catch (e) {}
  try {
    ...
    const s = snap.val() || {};
    const list = s.adminEmails || s.adminEmail || "";
    String(list)
      .split(/[,;\s]+/)
      .map((x) => x.trim().toLowerCase())
      .filter(Boolean)
      .forEach((e) => emails.add(e));
    if (s.adminName) names.add(String(s.adminName).toLowerCase().trim());
  } catch (e) {}
  return { emails: emails, names: names };
}

function commentLooksAdmin(c, adminSet) {
  if (!c) return false;
  if (c.isAdmin) return true;
  const em = String(c.email || "").toLowerCase().trim();
  const nm = String(c.name || "").toLowerCase().trim();
  if (em && adminSet.emails.has(em)) return true;
  if (nm && adminSet.names.has(nm)) return true;
  return false;
}
...
async function loadComments(postId) {
  ...
  try {
    await initFirebase();
    const [snap, adminSet] = await Promise.all([
      firebaseDb.ref("comments/" + postId).once("value"),
      getAdminIdentitySet()
    ]);
    const obj = snap.val() || {};
    ...
    arr.forEach((c) => {
      if (!c.isAdmin && commentLooksAdmin(c, adminSet)) {
        firebaseDb.ref("comments/" + postId + "/" + c.id + "/isAdmin").set(true).catch(() => {});
        c.isAdmin = true;
      }
    });
    ...
    const replies = arr.filter((c) => c.parentId);
    const ordered = [];
    roots.forEach((r) => {
      ordered.push(r);
      replies
    ...
    replies.forEach((x) => {
      if (!ordered.find((o) => o.id === x.id)) ordered.push(x);
    });

    list.innerHTML = ordered
      .map((c) => {
        const isAdmin = commentLooksAdmin(c, adminSet);
        const badge = isAdmin
          ? `<span class="cmt-badge cmt-badge-admin">Admin</span>`
          : `<span class="cmt-badge cmt-badge-guest">Khách</span>`;
        const avatarCls = isAdmin ? "cmt-avatar is-admin" : "cmt-avatar";
        const avatarIcon = isAdmin ? "fa-shield-halved" : "fa-user";
        const replyHint = c.replyToName
          ? `<p class="cmt-reply-to">Trả lời <strong>@${escapeHtml(c.replyToName)}</strong></p>`
          : "";
        const body = formatCommentBody(c.text || "");
        const replyClass = c.parentId ? "cmt-item cmt-reply" : "cmt-item";
        return `
      <div class="${replyClass}" data-cmt-id="${escapeHtml(c.id)}">
        <div class="cmt-head">
          <span class="${avatarCls}"><i class="far ${avatarIcon}"></i></span>
          <strong class="cmt-name">${escapeHtml(c.name || "Ẩn danh")}</strong>
          ${badge}
        ...
        <div class="cmt-actions">
          <button type="button" class="cmt-reply-btn" data-reply-cmt="${escapeHtml(c.id)}" data-reply-name="${escapeHtml(c.name || "")}">
    ...
    list.querySelectorAll("[data-report-cmt]").forEach((btn) => {
      btn.addEventListener("click", () =>
        reportComment(postId, btn.getAttribute("data-report-cmt"), btn)
      );
    });
    list.querySelectorAll("[data-reply-cmt]").forEach((btn) => {
      btn.addEventListener("click", () => {
        startReply(btn.getAttribute("data-reply-cmt"), btn.getAttribute("data-reply-name") || "");
      });
    });
  } catch (err) {
...
function formatCommentBody(text) {
  ...
  return safe.replace(/@([A-Za-z0-9_\u00C0-\u024F\u1E00-\u1EFF.]+)/g, '<span class="cmt-mention">@$1</span>');
}

function startReply(cmtId, name) {
  const idEl = document.getElementById("cmtReplyToId");
  const nameEl = document.getElementById("cmtReplyToName");
  const bar = document.getElementById("cmtReplyBar");
  const label = document.getElementById("cmtReplyLabel");
  const textEl = document.getElementById("cmtText");
  if (idEl) idEl.value = cmtId || "";
  if (nameEl) nameEl.value = name || "";
  if (bar) bar.classList.add("is-on");
  if (label) label.innerHTML = "Đang trả lời <strong>@" + escapeHtml(name || "") + "</strong>";
  if (textEl) {
    const tag = name ? "@" + name + " " : "";
    if (!textEl.value.includes(tag)) {
      textEl.value = tag + (textEl.value || "");
    }
    textEl.focus();
  }
...
async function submitComment(postId) {
  ...
  const replyToId = (document.getElementById("cmtReplyToId")?.value || "").trim();
  const replyToName = (document.getElementById("cmtReplyToName")?.value || "").trim();
  ...
  try {
    ...
    const asAdmin = isCommentAdminUser();
    const payload = {
      name: name.slice(0, 40),
      email: email.slice(0, 120),
      text: text.slice(0, 500),
      createdAt: Date.now(),
      approved: asAdmin ? true : false,
      reported: false,
      reportCount: 0,
      isAdmin: asAdmin
    };
    if (replyToId) {
      ...
      payload.replyToName = replyToName.slice(0, 40);
    }
    ...
    if (typeof showToast === "function") {
      showToast(
        asAdmin
          ? "Đã gửi bình luận (Admin - hiện ngay)."
          : "Đã gửi bình luận. Nội dung sẽ hiển thị sau khi admin duyệt.",
        "success"
      );
    }
