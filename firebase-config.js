...
/**
 * Lấy N bài mới nhất (index) — không tải cả node.
 * @param {"photos"|"videos"} collection
 * @param {number} limit
 */
...
/**
 * Phân trang list từ Firebase (mới → cũ).
 * @param {"photos"|"videos"} collection
 * @param {{ limit?: number, beforeTs?: number|null }} opts
 *   beforeTs: chỉ lấy bài có createdAt < beforeTs (trang sau)
 * @returns {{ items: array, hasMore: boolean, nextBefore: number|null }}
 */
async function fetchPostsPage(collection, opts) {
  ...
  // +1 để biết còn trang sau; nếu có beforeTs thì +1 để loại phần tử biên
  ...
  // endAt(beforeTs-1) đã loại biên; vẫn cắt đúng limit
...
/**
 * type=all: lấy từ photos + videos, merge theo createdAt, phân trang đơn giản.
 * Trang 1: lấy limit*2 mỗi bên rồi merge. Trang sau dùng beforeTs trên cả 2.
 */
...
/**
 * Tăng views / downloads trên photos hoặc videos (transaction).
 * Cần Anonymous Auth (hoặc admin) vì rules yêu cầu auth != null để ghi.
 */
