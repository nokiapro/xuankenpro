...
/** Lưu / đọc chuỗi cursor theo trang (session) để Prev hoạt động */
...
function renderPagination(el, opts) {
  ...
  if (page <= 1) {
    ...
    addLink(buildPageUrl(1), "‹", false, false, "page-nav");
  } else {
    addLink(buildPageUrl(page - 1, null, prevBefore), "‹", false, false, "page-nav");
  }
  ...
  if (page > 2) {
    items.push(`<span class="page-btn dots">…</span>`);
  }
  if (page > 1) {
...
(async function init() {
  ...
  try {
    ...
    // Cursor trang hiện tại: từ URL hoặc session (khi Prev)
    ...
    if (needClientFilter) {
      // Tag / search: vẫn cần quét (có thể tối ưu index sau). Giới hạn batch hợp lý.
      let pool = [];
      ...
      // Phân trang thật từ Firebase — 8 bài / trang
    ...
    // Lưu cursor cho trang kế (để Prev từ trang sau)
    ...
    // Sidebar ngẫu nhiên: chỉ lấy sample nhẹ (6+6), không full DB
    const rl = document.getElementById("randomList");
    if (rl) {
      try {
